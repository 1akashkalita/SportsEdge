...
**v3.0 Local Dashboard — Phase 1 (Foundation & Data Layer) complete (2026-06-24).** The Flask-on-3.14.0a2 tech choice (D-06) is proven live, so the stdlib `http.server` fallback stays documented-only. `scripts/dashboard.py` launches a one-command localhost server bound to **127.0.0.1 only** (port 8787, overridable via `--port`/`DASHBOARD_PORT`, browser auto-open, `use_reloader=False`); `scripts/dashboard_data.py` is a strictly read-only data layer (JSON-first + `read_only=True/data_only=True` workbook reads that leave source bytes byte-identical) that tolerates a mid-write atomic swap by returning last-known-good without raising (D-01), matches the runner's naive-local `today_str()` (D-02), and derives the updating-badge/last-updated signals from live+fresh cooperative-lock pids; the dark Pico.css shell (`templates/base.html` + `index.html`) renders the extensible nav with reserved Calibration/Line-changes/Live stubs and autoescape-on. **DASH-01..04 verified 8/8 must-haves; code-review blocker CR-01 resolved (narrowed exception handler).** Pending: 2 live human-UAT items (browser visual shell, off-box network-isolation curl) tracked in `01-HUMAN-UAT.md`.

**v3.0 Local Dashboard — Phase 2 (Read Views) complete (2026-06-24).** All three read-only pages render over the Phase 1 data layer: three accessors in `dashboard_data.py` (`get_today_board` / `get_all_slips` / `get_history_data`) shape the data; `dashboard.py` wires `/`, `/slips`, `/history` behind a shared `_freshness_context()`; templates deliver the Today master table (filter + EV-sort, VIEW-01), the date-grouped expandable Slips list with two-tier why-paired insight (VIEW-02), and the History page (overall + per-sport W/L, A/B/C/UNKNOWN tier breakdown, daily/weekly Chart.js bankroll/ROI chart, VIEW-03). XSS invariant holds (no `| safe` on workbook data; chart series via `| tojson`). **VIEW-01..03 verified 5/5 success criteria.** A code-review + verification blocker — **CR-01**: `/slips` re-opened both per-sport workbooks per slip (~184s on the 88-slip superset) — was fixed during this phase by indexing Correlated Parlays once per distinct date and skipping the settle-sleep on the read-only fan-out (**184s → 1.86s**), pinned by a once-per-date regression test (15 dashboard-view tests GREEN, Phase-1 dashboard regressions still GREEN). Pending: 3 live browser human-UAT items (Today sort/filter, Slips expand, History chart) tracked in `02-HUMAN-UAT.md`. **Next: Phase 3 — Safe Actions** (`/gsd-discuss-phase 3`).
...
- ✓ Read-only dashboard views — Today board (props/picks by platform × sport with EV/model-prob/edge/confidence, client-side platform/sport/status filter + EV-descending sort, dimmed skipped rows, VIEW-01), Slips list (date-grouped expandable rows with status/payout/legs + two-tier "why-paired" insight: stored Correlated-Parlays Reasoning/Group else derived rationale, VIEW-02), and History page (W/L overall + per-sport, A/B/C/UNKNOWN confidence-tier breakdown, daily/weekly Chart.js bankroll/ROI chart on pinned CDN+SRI, VIEW-03) — all rendered read-only over the Phase 1 data layer; XSS-safe (zero `| safe` on workbook data, chart series via `| tojson`). Code-review blocker CR-01 fixed: the /slips Correlated-Parlays lookup is indexed once per distinct date with a read-only fast delay (184s → 1.86s), pinned by a once-per-date regression test — Phase 2 (v3.0) (15 dashboard-view tests GREEN; 3 live browser human-UAT items pending in 02-HUMAN-UAT.md)
