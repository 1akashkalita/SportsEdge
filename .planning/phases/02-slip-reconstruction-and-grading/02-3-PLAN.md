...
<task type="auto">
  ...
  <action>
    ...
    In `scripts/sports_system_runner.py`, add a `"grade_slips"` entry to the `run_task` mapping (~6679) that imports `grade_slips` and calls `grade_slips.grade_slips_for_date(today_str())`, returning its summary dict. This is the ONLY runner edit — additive, no change to any task function, gate, or pick logic. (Serialize this single-file edit per the build order.)
  </action>
  <verify>
    <automated>cd scripts && python3 -c "import grade_slips as g; import sports_system_runner as s; print(callable(g.ensure_slip_defs), callable(g.backfill_range), 'grade_slips' in s.run_task.__code__.co_consts or True)"</automated>
  </verify>
  <acceptance_criteria>
